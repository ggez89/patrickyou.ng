...
export default function AsciiSky() {
  ...
  useEffect(() => {
    ...
    const onMove = (clientX: number, clientY: number) => {
      ...
      if (dist > 15 || paintStrokes.current.length === 0) {
        ...
        const newStroke: MousePaintStroke = {
          x: clientX,
          y: clientY,
          radius: Math.random() * 60 + 110, // 110-170px gorgeous painting radius
          intensity: 1.0,
    ...
    const render = () => {
      ...
      time.current += 0.016; // Stable delta simulation clock
        ...
        for (let c = 0; c < cols; c++) {
          ...
          if (bluePaintVal > 0.02) {
            ...
            if (isActive) {
              ...
              // Ignite subtle sparkling blue stars of varying character shapes in the empty void
